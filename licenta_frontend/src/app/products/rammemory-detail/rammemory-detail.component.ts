...
@Component({
  selector: 'app-rammemory-detail',
  templateUrl: './rammemory-detail.component.html',
  styleUrls: ['./rammemory-detail.component.scss']
})
export class RammemoryDetailComponent {
  id: number = 0;
  ramMemory?: RamMemory;
  stars:number =1;
  ...
  ngOnInit(): void {
    ...
    this.geRamMemoryById(this.id);
    this.loginService.getUser().subscribe(account=>{
      this.account=account;
    });
  }
  geRamMemoryById(id: number): void {
    this.rammemoryService
      .getRamMemoryById(id)
      .subscribe((ramMemory) => {
        this.ramMemory = ramMemory
        this.reviews = ramMemory.reviews
      });
  }
