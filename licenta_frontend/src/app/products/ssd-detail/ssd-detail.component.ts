...
@Component({
  selector: 'app-ssd-detail',
  templateUrl: './ssd-detail.component.html',
  styleUrls: ['./ssd-detail.component.scss']
})
export class SsdDetailComponent {
  id: number = 0;
  ssd?: Ssd;
  stars:number =1;
  ...
  ngOnInit(): void {
    ...
    this.getSsdById(this.id);
    this.loginService.getUser().subscribe(account=>{
      this.account=account;
    });
  }
  getSsdById(id: number): void {
    this.ssdService
      .getSsdById(id)
      .subscribe((ssd) => {
        this.ssd = ssd
        this.reviews = ssd.reviews
      });
  }
